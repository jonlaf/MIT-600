...
def is_valid_word(word, hand, word_list):
    ...
    if word_list.count(word) == 0:
        ...
        for i in range(0, len(word)):
            if temphand.get(word[i]):
                temphand[word[i]] -= 1
        if calculate_handlen(hand) - calculate_handlen(temphand) == len(word):
            return True
        else:
            return False
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while word != '.' and calculate_handlen(newhand) > 0:
        if is_valid_word(word, hand, word_list) == False:
            print "either that is not a word or you don't have the letters to make it."
